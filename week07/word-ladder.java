...
//        return func2(beginWord, endWord, wordList);
    ...
    private int func2(String beginWord, String endWord, List<String> wordList) {
    ...
    private int func1(String beginWord, String endWord, List<String> wordList) {
        // 首先将单词字典放到hashSet中，方便我们通过O(1)的时间查找
        Set<String> wordSet = new HashSet<>(wordList);
    ...
    public boolean findPath(String currentWord, String endWord,
        ...
        for (int i = 0; i < currentWordArray.length; i++) {
            ...
            for (char k = 'a'; k <= 'z'; k++) {
                ...
                // 如果单词表里包含这个词并且我们没有访问过这个词
                if (wordSet.contains(newWord) && !visited.contains(newWord)) {
                    // 将其加入到队列中，并且更新visite哈希表。这里不用担心他会把同一层次的节点加进来，因为同一层次的节点在上一层次的遍历肯定全部都加进来了，所以肯定通不过！visited
                    // 为什么不能加入同一层次的节点？因为和同一层次的连一条边没有意义，这样就并不是最短的路径了。
                    queue.offer(newWord);
                    visited.add(newWord);
                }
            }
